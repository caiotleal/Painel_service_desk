...
export default function App() {
  ...
  // --- CHECK SLA CORRIGIDO (DATA ROBUSTA) ---
  const checkSLA = (ticket) => {
    ...
    // 1. Verifica se é Timestamp do Firestore (Objeto com .toDate)
    if (dataRaw?.toDate) {
       prazo = dataRaw.toDate().getTime();
    } 
    // 2. Se não, tenta converter string/numero normal
    else {
    ...
    // Se a data for inválida, retorna sem prazo
    if (isNaN(prazo)) return 'sem-prazo';

    const diferencaHoras = (prazo - agora) / (1000 * 60 * 60);

    // Regras de Cores
    if (diferencaHoras < 0) return 'vencido';     // Vermelho (Já passou)
    if (diferencaHoras <= 2) return 'alerta';    // Amarelo (Aumentei para 24h para você ver funcionar)
    return 'em-dia';                              // Verde (> 24h)
  };

  // --- FUNÇÃO DE LIMPEZA DE TICKETS QUE NÃO SÃO ABERTOS ---
  const limparTicketsNaoAbertos = async (lista) => {
    lista.forEach(async (t) => {
      const status = String(t.status || t.Status || "").toLowerCase().trim();
      // Se NÃO for aberto nem open, removemos do banco
      if (status !== "" && status !== "aberto" && status !== "open") {
        try {
          await deleteDoc(doc(db, "tickets", String(t.id)));
          console.log(`[CleanUp] Ticket ${t.id} removido por status: ${status}`);
        } catch (e) {
  ...
  useEffect(() => {
    ...
    const unsubT = onSnapshot(collection(db, "tickets"), (snap) => {
      ...
      // Filtramos localmente para exibição imediata
      ...
      // Disparamos a limpeza no banco de dados para os outros
      limparTicketsNaoAbertos(data);
    });
  ...
  const onDragEnd = async (res) => {
    ...
    if (colDest.tipo === 'action') {
      ...
      if (colDest.id !== colOrigem.id) {
        ...
        enviarNotificacao(WEBHOOK_MOVIMENTACAO, "🔄 Movimentação de Ticket", `Ticket movido para: **${colDest.id}**`, ticket, "Accent");
      }
    } else {
  ...
  return (
    <div className={`${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'} h-screen w-full flex flex-col overflow-hidden font-sans`}>
      <header className={`${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} h-16 border-b px-6 flex justify-between items-center z-50 shrink-0`}>
        <div className="flex items-center gap-8">
          ...
          <div className="flex -space-x-2">
            {analistas.map((a, i) => (
              <div key={i} title={`${a.nome} (${a.email})`} className="w-8 h-8 rounded-full bg-blue-600 border-2 border-slate-900 flex items-center justify-center text-[10px] text-white font-black uppercase shadow-md cursor-help transition-transform hover:scale-110 hover:z-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            ...
            {menuOpen && (
              <div className="absolute right-0 top-12 w-48 bg-white dark:bg-slate-800 rounded-xl shadow-2xl border border-slate-200 dark:border-slate-700 p-2 z-[100] animate-in fade-in zoom-in-95">
                <div className="px-3 py-2 text-[10px] font-bold uppercase text-slate-500 border-b border-slate-700/30 mb-2">Configurações</div>
          ...
          <button onClick={handleLogout} className="text-[10px] font-black uppercase text-slate-400 hover:text-red-500 transition-colors border border-slate-700/30 px-3 py-1.5 rounded-lg hover:bg-red-500/10">Sair</button>
        </div>
      </header>
                      ...
                      {list.map((t, index) => {
                        ...
                        return (
                          <Draggable key={`${t.id}-${col.id}`} draggableId={`${t.id}-${col.id}`} index={index}>
                            {(p) => (
                              <div ref={p.innerRef} {...p.draggableProps} {...p.dragHandleProps} className={`relative p-4 rounded-xl shadow-md border transition-all duration-300 ${darkMode ? 'bg-slate-800' : 'bg-white'} ${col.tipo === 'backlog' ? 'w-full min-h-[70px]' : 'w-[300px] min-h-[70px]'} ${sla === 'vencido' ? 'border-l-4 border-l-red-600' : sla === 'alerta' ? 'border-l-4 border-l-yellow-500' : sla === 'em-dia' ? 'border-l-4 border-l-green-500' : 'border-l-4 border-slate-700'} ${isExpanded ? 'z-[100]' : 'z-10'}`}>
                                <div className="flex justify-between items-start w-full">
                                  <div className="flex flex-col gap-0.5 overflow-hidden w-full">
                                    <div className="flex items-center gap-2">
                                      ...
                                      {col.tipo === 'action' && t.updated_by && <div className="text-[8px] font-black text-blue-400 bg-blue-500/10 px-1.5 py-0.5 rounded-full uppercase italic">{t.updated_by}</div>}
                                    </div>
                                    <h3 className={`text-[12px] font-bold leading-tight truncate ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>{t.subject}</h3>
                                  </div>
                                  <button onClick={() => toggleStar(t.id, t.prioridade_painel)} className="ml-2 outline-none"><Star size={16} fill={t.prioridade_painel ? "#ef4444" : "none"} className={t.prioridade_painel ? "text-red-500 scale-110" : "text-slate-400"} /></button>
                                </div>
                                <div className="mt-2 flex">
                                  <div onMouseEnter={() => setHoveredCard(t.id)} onMouseLeave={() => setHoveredCard(null)} onClick={() => setFixedCards(prev => ({ ...prev, [t.id]: !prev[t.id] }))} className={`flex items-center gap-1 px-2 py-0.5 rounded cursor-pointer transition-colors ${fixedCards[t.id] ? 'bg-blue-600 text-white' : (darkMode ? 'bg-slate-700/50 text-slate-400 hover:text-white' : 'bg-slate-100 text-slate-500')}`}>
                                    <ChevronDown size={14} className={`transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                                    <span className="text-[8px] font-black uppercase">{fixedCards[t.id] ? 'Fixado' : 'Detalhes'}</span>
                                  </div>
                                </div>
                                {isExpanded && (
                                  <div className={`absolute left-0 top-[90%] w-full p-4 rounded-b-xl shadow-2xl border-x border-b animate-in fade-in zoom-in-95 duration-200 z-[110] ${darkMode ? 'bg-slate-800 border-slate-700 shadow-black/50' : 'bg-white border-slate-200 shadow-slate-300'}`}>
                                    <div className="flex flex-col gap-2 max-h-[250px] overflow-y-auto pr-1 scrollbar-thin">
                                      {t.group && <div className="flex flex-col border-b border-blue-500/30 pb-1"><span className="text-[7px] font-black uppercase text-blue-400 tracking-tighter">Grupo</span><span className={`text-[10px] font-bold ${darkMode ? 'text-blue-200' : 'text-blue-700'}`}>{t.group}</span></div>}
                                      {Object.entries(t).map(([k, v]) => {
                                        if (['id', 'subject', 'prioridade_painel', 'fluxo_ativo', 'status_interno', 'priority', 'group', 'updated_by'].includes(k) || !v) return null;
                                        return ( <div key={k} className="flex flex-col border-b border-slate-700/30 pb-1 last:border-0"><span className="text-[7px] font-black uppercase text-blue-500 tracking-tighter">{k.replace(/_/g, ' ')}</span><span className={`text-[10px] font-medium break-words leading-tight ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>{String(v)}</span></div> );
                                      })}
                                    </div>
                                  </div>
                                )}
      ...
      {/* QUADROS FLUTUANTES */}
      {urgentes.length > 0 && (
        <DraggablePopup nodeRef={nodeRefUrgente} handle=".handle">
          <div ref={nodeRefUrgente} className="fixed bottom-10 right-10 w-72 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border-2 border-red-600 overflow-hidden z-[200]">
            <div className="handle bg-red-600 p-3 flex items-center justify-between cursor-grab text-white font-black text-[10px] uppercase active:cursor-grabbing"><span>Prioridade Alta</span><Star size={14} className="animate-pulse fill-white"/></div>
            <div className="p-3 max-h-40 overflow-y-auto space-y-2 dark:bg-slate-800 bg-red-50/30">
              {urgentes.map(t => ( <div key={t.id} className="text-[9px] font-bold border-b border-slate-700/30 pb-1 last:border-0 truncate flex justify-between"><a href={`https://engage-bz.freshdesk.com/a/tickets/${t.id}`} target="_blank" rel="noopener noreferrer" className="hover:underline">{t.subject}</a><span className="text-red-500 opacity-60 ml-2">#{t.id}</span></div> ))}
            </div>
          </div>
        </DraggablePopup>
      )}

      {slaVermelho.length > 0 && (
        <DraggablePopup nodeRef={nodeRefVermelho} handle=".handle-vermelho">
          <div ref={nodeRefVermelho} className="fixed bottom-10 right-[310px] w-72 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border-2 border-red-900 overflow-hidden z-[190]">
            <div className="handle-vermelho bg-red-900 p-3 flex items-center justify-between cursor-grab text-white font-black text-[10px] uppercase active:cursor-grabbing"><span>SLA Estourado</span><ShieldAlert size={14} className="animate-pulse"/></div>
            <div className="p-3 max-h-40 overflow-y-auto space-y-2 dark:bg-slate-800 bg-red-50/30">
              {slaVermelho.map(t => ( <div key={t.id} className="text-[9px] font-bold border-b border-slate-700/30 pb-1 last:border-0 truncate flex justify-between"><a href={`https://engage-bz.freshdesk.com/a/tickets/${t.id}`} target="_blank" rel="noopener noreferrer" className="hover:underline text-red-400">{t.subject}</a><span className="text-red-500 opacity-60 ml-2">#{t.id}</span></div> ))}
            </div>
          </div>
        </DraggablePopup>
      )}
      ...
      {slaAmarelo.length > 0 && (
        <DraggablePopup nodeRef={nodeRefAmarelo} handle=".handle-amarelo">
          <div ref={nodeRefAmarelo} className="fixed bottom-10 right-[610px] w-72 bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border-2 border-yellow-500 overflow-hidden z-[180]">
            <div className="handle-amarelo bg-yellow-500 p-3 flex items-center justify-between cursor-grab text-slate-900 font-black text-[10px] uppercase active:cursor-grabbing"><span>Atenção SLA (24h)</span><Clock size={14} className="animate-bounce"/></div>
            <div className="p-3 max-h-40 overflow-y-auto space-y-2 dark:bg-slate-800 bg-yellow-50/30">
              {slaAmarelo.map(t => ( <div key={t.id} className="text-[9px] font-bold border-b border-slate-700/30 pb-1 last:border-0 truncate flex justify-between"><a href={`https://engage-bz.freshdesk.com/a/tickets/${t.id}`} target="_blank" rel="noopener noreferrer" className="hover:underline text-yellow-500">{t.subject}</a><span className="text-yellow-600 opacity-60 ml-2">#{t.id}</span></div> ))}
            </div>
          </div>
        </DraggablePopup>
      )}
